; NOTE: This has been made to conform to the released 7800 ROM.
; The original pre-release version dated 1984-04-23 can be found at:
...
CODEDIF   EQU     ROMCODE-RAMCODE        ;DIFFERENCE BETWEEN OLD AND ...
...
CSALOOP   LDA     $FF00,Y                ;GET HI PAGE INTO ACC
          ...
          LDA     #<(S-CODEDIF)          ;SET UP FOR THE RETURN
...
CSCSLOOP  JSR     CSCHKDLI-CODEDIF       ;CHECK FOR COMING DLI
          ...
          LDA     #<(T-CODEDIF)          ;SET UP FOR THE RETURN MARCH
          STA     CSCMOD1+1-CODEDIF
          LDA     #>(T-CODEDIF)
          STA     CSCMOD1+2-CODEDIF
...
HSEROFF   DB      >(REG0),>(REG2),>(REG4),>(REG6)
          DB      >(REG8),>(REG10),>(REG12),>(REG14)
...
FATAL_    LDA     #$1D                   ;THERE HAS BEEN SOMETHING BA...
...
CTF       LDX     #$55                   ;TEST X AND Y LOADS, STORES
          ...
          CMP     $2121
          BNE     CPUERR0
          LDA     #<CTCONT               ;TEST (IND), ONLY JMP USES
          STA     $F0
          LDA     #>CTCONT
          STA     $F1
          JMP     ($F0)                  ;(IND)
...
CTJSR     TSX                            ;SEE WHERE STACK IS
          ...
          CMP     #>(CTJSRRET-1)
          BNE     CPUERR1
          LDA     #>(RAMTEST-1)          ;PUT START OF CODE AS RETURN
          PHA
          LDA     #<(RAMTEST-1)
          PHA
          RTS                            ;DO IT
...
*  DISPLAY LISTS
ROMDLIST  DB      <(RAMGRAPH),$1F,>(RAMGRAPH),$BB,$00,$00        ;4 BY...
RDL5BYTE  DB      <(RAMGRAPH),$40,>(RAMGRAPH),$1F,$BB,$00,$00    ;5 BY...
RDLFUJI1  DB      <(RAMGRAPH+ROMFUJI1-ROMGRAPH),$1C,>(RAMGRAPH),$4A,$00,$00
RDLFUJI2  DB      <(RAMGRAPH+ROMFUJI2-ROMGRAPH),$1C,>(RAMGRAPH),$4A,$00,$00
RDLFUJI3  DB      <(RAMGRAPH+ROMFUJI3-ROMGRAPH),$1C,>(RAMGRAPH),$48,$00,$00
RDLFUJI4  DB      <(RAMGRAPH+ROMFUJI4-ROMGRAPH),$1B,>(RAMGRAPH),$46,$00,$00
RDLFUJI5  DB      <(RAMGRAPH+ROMFUJI5-ROMGRAPH),$19,>(RAMGRAPH),$42,$00,$00
RDLFUJI6  DB      <(RAMGRAPH+ROMFUJI6-ROMGRAPH),$17,>(RAMGRAPH),$3E,$00,$00
RDLFUJI7  DB      <(RAMGRAPH+ROMFUJI7-ROMGRAPH),$17,>(RAMGRAPH),$3E,$00,$00
RDLRACE   DB      <(RAMGRAPH+ROMSTRIP-ROMGRPH3),$2C,>(RAMGRAPH+$300),$00
          DB      <(RAMGRAPH+ROMSTRIP-ROMGRPH3),$2C,>(RAMGRAPH+$300),$50,$00,$00
RDLRACEL  DB      <(RAMGRAPH+ROMSTRIP-ROMGRPH3),$2C,>(RAMGRAPH+$400),$00
          DB      <(RAMGRAPH+ROMSTRIP-ROMGRPH3),$2C,>(RAMGRAPH+$400),$50,$00,$00
RDLINE01  DB      <(RAMGRAPH+ROMLINE1-ROMGRAPH),$2D,>(RAMGRAPH+$000),$28,$00,$00
RDLINE02  DB      <(RAMGRAPH+ROMLINE2-ROMGRAPH),$2D,>(RAMGRAPH+$000),$28,$00,$00
RDLINE03  DB      <(RAMGRAPH+ROMLINE3-ROMGRAPH),$2D,>(RAMGRAPH+$000),$28,$00,$00
RDLINE04  DB      <(RAMGRAPH+ROMLINE4-ROMGRAPH),$2D,>(RAMGRAPH+$000),$28,$00,$00
RDLINE05  DB      <(RAMGRAPH+ROMLINE1-ROMGRAPH),$2D,>(RAMGRAPH+$100),$28,$00,$00
RDLINE06  DB      <(RAMGRAPH+ROMLINE2-ROMGRAPH),$2D,>(RAMGRAPH+$100),$28,$00,$00
RDLINE07  DB      <(RAMGRAPH+ROMLINE3-ROMGRAPH),$2D,>(RAMGRAPH+$100),$28,$00,$00
RDLINE08  DB      <(RAMGRAPH+ROMLINE4-ROMGRAPH),$2D,>(RAMGRAPH+$100),$28,$00,$00
RDLINE09  DB      <(RAMGRAPH+ROMLINE1-ROMGRAPH),$2D,>(RAMGRAPH+$200),$28,$00,$00
RDLINE10  DB      <(RAMGRAPH+ROMLINE2-ROMGRAPH),$2D,>(RAMGRAPH+$200),$28,$00,$00
RDLINE11  DB      <(RAMGRAPH+ROMLINE3-ROMGRAPH),$2D,>(RAMGRAPH+$200),$28,$00,$00

*  DISPLAY LIST LIST
ROMDLL    DB      $0F,>(RAMDLIST),<(RAMDLIST+RDL5BYTE-ROMDLIST)  ;5 BY...
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $03,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $85,>(RAMDLIST),<(RAMDLIST+RDLFUJI1-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI2-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI3-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI4-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI5-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI6-ROMDLIST)
          DB      $05,>(RAMDLIST),<(RAMDLIST+RDLFUJI7-ROMDLIST)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)    ;CENTER SPACE
          DB      $01,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)   ;ATAR...
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE01-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE02-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE03-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE04-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE05-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE06-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE07-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE08-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE09-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE10-ROMDLIST)
          DB      $02,>(RAMDLIST),<(RAMDLIST+RDLRACE-ROMDLIST)
          DB      $00,>(RAMDLIST),<(RAMDLIST+RDLINE11-ROMDLIST)
          DB      $01,>(RAMDLIST),<(RAMDLIST+RDLRACEL-ROMDLIST)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)        ;TRAILING SPA...
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
          DB      $0F,>(RAMDLIST),<(RAMDLIST+$00)
...
ENDROM2   NOP
          ...
          ORG     $FFF8
          DB      $2D                    ;CHECKSUM, MAKES EOR CHECK...
